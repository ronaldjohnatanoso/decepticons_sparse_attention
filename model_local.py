...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Compute attention scores only within the local window
        for i in range(T):
            ...
            att_scores = (q[:, :, i:i+1] @ k[:, :, start:end].transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # (B, nh, 1, w)
            
            # Apply causal mask within the window
            causal_mask = torch.tril(torch.ones(1, window_length, device=x.device), diagonal=0).bool()  # (1, w)
            ...
            # Compute weighted sum of values within the window
            y[:, :, i:i+1] = att_weights @ v[:, :, start:end]  # (B, nh, 1, hs)
